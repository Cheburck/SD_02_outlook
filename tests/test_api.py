...
class TestAuth:
    ...
    def test_register_duplicate_user(self):
        with TestClient(app) as client:
            user_data = {
                "login": f"dupuser_{os.urandom(4).hex()}",
                "firstName": "Duplicate",
                "lastName": "User",
                "password": "password123"
            }
    ...
    def test_login_success(self):
        with TestClient(app) as client:
            user_data = {
                "login": f"loginuser_{os.urandom(4).hex()}",
                "firstName": "Login",
                "lastName": "User",
                "password": "password123"
            }
...
class TestUsers:
    def test_create_user(self):
        with TestClient(app) as client:
            user_data = {
                "login": f"newuser_{os.urandom(4).hex()}",
                "firstName": "New",
                "lastName": "User",
                "password": "password123"
            }
    ...
    def test_get_user_by_login(self):
        with TestClient(app) as client:
            user_data = {
                "login": f"findme_{os.urandom(4).hex()}",
                "firstName": "Find",
                "lastName": "Me",
                "password": "password123"
            }
            client.post("/api/users", json=user_data)
            response = client.get(f"/api/users/login/{user_data['login']}")
            assert response.status_code == 200
            data = response.json()
    ...
    def test_search_users_by_name(self):
        with TestClient(app) as client:
            users = [
                {"login": f"john_doe_{os.urandom(2).hex()}", "firstName": "John", "lastName": "Doe", "password": "pass123"},
                {"login": f"jane_doe_{os.urandom(2).hex()}", "firstName": "Jane", "lastName": "Doe", "password": "pass123"},
                {"login": f"john_smith_{os.urandom(2).hex()}", "firstName": "John", "lastName": "Smith", "password": "pass123"},
            ]
            for user in users:
...
class TestFolders:
    ...
    def test_delete_folder(self):
        with TestClient(app) as client:
            token = self.get_auth_token()
            user_data = {
                "login": f"deleteowner_{os.urandom(4).hex()}",
                "firstName": "Delete",
                "lastName": "Owner",
                "password": "password123"
            }
            ...
            folder_response = client.post("/api/folders", json={"name": "ToDelete", "userId": user_id}, headers={"Authorization": f"Bearer {token}"})
            folder_id = folder_response.json()["id"]
            
            response = client.delete(f"/api/folders/{folder_id}", headers={"Authorization": f"Bearer {token}"})
            assert response.status_code == 204
            
            response = client.get(f"/api/folders/{folder_id}", headers={"Authorization": f"Bearer {token}"})
            assert response.status_code == 404
...
class TestMessages:
    ...
    def test_get_message_by_subject(self):
        with TestClient(app) as client:
            token, folder_id = self.setup_message_test()
            create_response = client.post(f"/api/folders/{folder_id}/messages", json={
                "subject": "UniqueFileName.txt",
                "body": "Find this file",
                "sender": "sender@example.com",
                "recipient": "recipient@example.com"
            })
            
            response = client.get(f"/api/messages?subject=UniqueFileName.txt")
            assert response.status_code == 200
            data = response.json()
            assert data["subject"] == "UniqueFileName.txt"
